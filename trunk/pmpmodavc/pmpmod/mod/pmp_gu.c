...
void pmp_gu_start()
	...
	memset(pmp_gu_avc_buffer, 0, 4 * 512 * 512);
...
void pmp_gu_draw(unsigned int aspect_ratio, unsigned int zoom, unsigned int luminosity_boost, unsigned int show_interface, unsigned int show_subtitle, unsigned int subtitle_format, unsigned int frame_number, void *video_frame_buffer)
	...
	pmp_gu_load(pmp_gu_avc_buffer, filter);
	...
	*/
	...
	if (show_subtitle && gu_cur_font!=0)
		{
		int flags = 0;
		int pwidth = 0;
		...
		if (pmp_sub_parse_get_frame(&subtitle_parser[show_subtitle-1], &frame, frame_number)==0)
			{
				if (frame!=0)
					{
					if ((subtitle_format==1) && (gu_cur_font->haveflags&GU_FONT_HAS_UNICODE_CHARMAP))
						{
						flags = FLAG_UTF8;
					...
					gu_font_print( (480-pwidth)/2, 270-frame->p_num_lines*gu_font_height(), flags | FLAG_ALIGN_CENTER, frame->p_string);
