...
// jonny 2005-11-11
static int saved_decode_slice(MpegEncContext *s){
...
//printf("%d %d %06X\n", ret, get_bits_count(&s->gb), show_bits(&s->gb, 24));
			...
			// jonny 2005-11-16
			if (me_idct_start_row != me_idct_mb_y && me_unused())
				{
				me_idct_row_run_in_background();
				}


			// jonny 2005-11-11
			s->block = me_idct_new_macroblock();
...
static int decode_slice(MpegEncContext *s)
	...
	void *saved_block = s->block;
	int result = saved_decode_slice(s);
	s->block = saved_block;
