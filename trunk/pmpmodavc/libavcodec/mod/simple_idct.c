...
void simple_idct_put(uint8_t *dest, int line_size, DCTELEM *block)
	{
	me_idct_add(1, dest, line_size, me_idct_block_index(block));
	}

void simple_idct_add(uint8_t *dest, int line_size, DCTELEM *block)
	{
	me_idct_add(0, dest, line_size, me_idct_block_index(block));
	}
...
// jonny 2005-11-11
void saved_simple_idct_put(uint8_t *dest, int line_size, DCTELEM *block)
{
    int i;
    for(i=0; i<8; i++)
...
void saved_simple_idct_add(uint8_t *dest, int line_size, DCTELEM *block)
{
    int i;
    for(i=0; i<8; i++)
        idctRowCondDC(block + i*8);
