...
utils.c, avcodec_default_get_buffer
...
STRIDE_ALIGN need to be 64

av_malloc(... + STRIDE_ALIGN)  (needed for data[i] alignment)
*/
...
int avcodec_default_get_buffer(AVCodecContext *s, AVFrame *pic){
    ...
    if(buf->base[0]){
        ...
        for(i=0; i<3; i++){
            ...
            buf->base[i]= av_malloc((buf->linesize[i]*h>>v_shift)+STRIDE_ALIGN);
